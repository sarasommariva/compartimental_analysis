function [k1x,k2x,k3x,k4x,Cf0x,Cp0x,Vbx,Cx,Cxdata,relerr,iter,cont,nit] = ...
    reconstruction_Skf_New(Cdata,Ca,Vi,t,t0,C0,k1,k2,k3,k4,Cf0,Cp0,Vb)
...
%         Kx = num2cell([k1,k2,k3,k4,Cf0,Cp0,Vb]+(rand(1,7)-4/5).*[k1,k2,k3,k4,Cf0,Cp0,Vb]);%[(rand(1,4)-4/5).*[k1,k2,k3,k4],0.1*Cf0,0.1*Cp0,(rand(1)-4/5)*Vb]);
%         [k1x,k2x,k3x,k4x,Cf0x,Cp0x,Vbx] = deal(Kx{:});
% end
        k1x = k1;
        k2x = k2;
        k3x = k3;
...
while relerr>toll
    ...
    aux1 = dF_dk1(Mx,Ca,t0,t,alphax)';
    auxM = dF_dM(k1x,Mx,Ca,t0,[Cf0x,Cp0x],t,alphax)';
    D = [aux1 auxM];
    D = [D(:,1),-D(:,2),D(:,3)-D(:,2),D(:,4)-D(:,5)]; %qui aggiungo altri parametri
    
    auxcf0 = dF_dCf(k1x,Mx,Ca,t0,[Cf0x,Cp0x],t,alphax)'; %Cf0
    auxcp0 = dF_dCp(k1x,Mx,Ca,t0,[Cf0x,Cp0x],t,alphax)'; %Cp0
    auxVb0 = dF_dVb(k1x,Mx,Ca,t0,[Cf0x,Cp0x],t,alphax)'; % Vb
    D = [D, auxcf0 , auxcp0, auxVb0];
    %D = [D, (-[1,1]*concentration(k1x,Mx,Ca,t0,[Cf0x,Cp0x],t)+Ca(t))']; %Vb
    ...
    k1x = x(1); k2x = x(2); k3x = x(3); k4x = x(4); Cf0x = x(5); Cp0x = x(6); Vbx = x(7);
    alphax = [1-Vbx,1-Vbx-Vi];
...
%                     Vbx = 0;
%                 case narg_opt % simulation
%                     Kx = num2cell([k1,k2,k3,k4,Cf0,Cp0,Vb]+(rand(1,7)-4/5).*[k1,k2,k3,k4,Cf0,Cp0,Vb]);%[(rand(1,4)-4/5).*[k1,k2,k3,k4],0.1*Cf0,0.1*Cp0,(rand(1)-4/5)*Vb]);%(rand(1,7)-4/5).*[k1,k2,k3,k4,Cf0,Cp0,Vb]);%,0.1*Cf0,0.1*Cp0,(rand(1)-4/5)*Vb]);%num2cell([k1,k2,k3,k4,Cf0,Cp0,Vb]+(rand(1,4)-4/5).*[k1,k2,k3,k4,Cf0,Cp0,Vb]);
%                     [k1x,k2x,k3x,k4x,Cf0x,Cp0x,Vbx] = deal(Kx{:});
%             end
            k1x = k1;
            k2x = k2;
